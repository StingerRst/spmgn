...
* @copyright (c) Mark D. Hamill (mark@phpbbservices.com, http://phpbbservices.com) 
...
// This software is designed to work with phpBB Version 3.0.11.
...
$weekly_digest_sql = (gmdate('w') == $config['digests_weekly_digest_day']) ? ' OR (' . $db->sql_in_set('user_digest_type', array(DIGEST_WEEKLY_VALUE)) . ')': '';
...
if ($gmt_day == 1)
{
	...
	$month_1st_begin_gmt = mktime(0, 0, 0, $gmt_month, 1, $gmt_year);
	$month_lastday_end_gmt = mktime(23, 59, 59, $gmt_month, date('t', $month_1st_begin_gmt), $gmt_year);
...
{
	$date_limit_sql = ' AND p.post_time >= ' . $month_1st_begin_gmt;
}
...
$sql_array = array(
	...
	'WHERE'		=> "f.forum_id = t.forum_id AND p.topic_id = t.topic_id 
...
foreach ($rowset as $row)
{
	...
	$messenger->assign_vars(array(
		...
		'L_DIGEST_PUBLISH_DATE'			=> sprintf($user->lang['DIGEST_PUBLISH_DATE'], date(str_replace('|','',$row['user_dateformat']), $recipient_time)),
...
function create_content (&$rowset, &$pm_rowset, &$user_row)
{
	...
	if ((sizeof($pm_rowset) != 0) && ($user_row['user_digest_show_pms'] == 1))
	{
		...
		foreach ($pm_rowset as $pm_row)
		{
			...
			$toc['pms'][$toc_pm_count]['message_subject'] = html_entity_decode($pm_row['message_subject']);
			...
			if ($pm_row['message_attachment'] > 0 && $user_row['user_digest_attachments'])
			{
				...
				while ($row3 = $db->sql_fetchrow($result3))
				{
					...
					if (substr($row3['mimetype'],0,6) == 'image/')
					{
						...
						$pm_text .= sprintf("%s<br /><em>%s</em> (%s KiB)<br />%s<img src=\"%s\" alt=\"%s\" title=\"%s\" />%s\n<br />%s", $row3['attach_comment'], $row3['real_filename'], $file_size, $anchor_begin, $board_url . "download/file.$phpEx?id=" . $row3['attach_id'] . $thumbnail_parameter, $row3['attach_comment'], $row3['attach_comment'], $anchor_end, $pm_image_text);
					}
					else
					{
						$pm_text .= ($row3['attach_comment'] == '') ? '' : '<em>' . $row3['attach_comment'] . '</em><br />';
						$pm_text .= 
			...
			$template->assign_block_vars('pm', array(
				...
				'PRIVATE_MESSAGE_LINK'		=> ($is_html) ? sprintf('<a href="%s?i=pm&amp;mode=view&amp;f=0&amp;p=%s">%s</a>', $board_url . 'ucp.' . $phpEx, $pm_row['msg_id'], $pm_row['msg_id']) . "\n" : html_entity_decode($pm_row['message_subject']) . "\n",
				'PRIVATE_MESSAGE_SUBJECT'	=> ($is_html) ? sprintf('<a href="%s?i=pm&amp;mode=view&amp;f=0&amp;p=%s">%s</a>', $board_url . 'ucp.' . $phpEx, $pm_row['msg_id'], html_entity_decode($pm_row['message_subject'])) . "\n" : html_entity_decode($pm_row['message_subject']) . "\n",
	...
	if (sizeof($rowset) != 0)
	{
		...
		else
		{
			...
			$sql3 = 'SELECT * FROM ' . DIGESTS_SUBSCRIBED_FORUMS_TABLE . '
				...
				
				{
					...
					case 'a':
						...
						foreach ($rowset as $key => $row)
						{
							...
							switch($user_row['user_post_sortby_type'])
							{
								...
								break;
								case 's':
									$post_subject[$key] = $row['post_subject'];
								break;
							}
						}
						switch($user_row['user_post_sortby_type'])
					...
					case 's':
						...
						foreach ($rowset as $key => $row)
						{
							$left_id[$key]  = $row['left_id'];
							$right_id[$key] = $row['right_id'];
							$topic_title[$key] = $row['topic_title'];
							switch($user_row['user_post_sortby_type'])
							{
								case 'a':
									$username_clean[$key] = $row['username_clean'];
		...
		foreach ($rowset as $post_row)
		{
			...
			// Exclude posts for monthly digests that occur after the end of the previous month
			if (($user_row['user_digest_type'] == DIGEST_MONTHLY_VALUE) && ($post_row['post_time'] > $month_lastday_end_gmt))
			{
				continue;
			}

			// Skip if post has less than minimum words wanted.
			if (($user_row['user_digest_min_words'] > 0) && ((truncate_words($post_row['post_text'], $user_row['user_digest_min_words'], true) < $user_row['user_digest_min_words']))) 
			{
				continue;
			}
			...
			if ($post_row['post_attachment'] > 0 && $user_row['user_digest_attachments'])
			{
				...
				while ($row3 = $db->sql_fetchrow($result3))
				{
					...
					if (substr($row3['mimetype'],0,6) == 'image/')
					{
						...
						$post_text .= sprintf("%s<br /><em>%s</em> (%s KiB)<br />%s<img src=\"%s\" alt=\"%s\" title=\"%s\" />%s\n<br />%s", $row3['attach_comment'], $row3['real_filename'], $file_size, $anchor_begin, $board_url . "download/file.$phpEx?id=" . $row3['attach_id'] . $thumbnail_parameter, $row3['attach_comment'], $row3['attach_comment'], $anchor_end, $post_image_text);
					}
					else
					{
						$post_text .= ($row3['attach_comment'] == '') ? '' : '<em>' . $row3['attach_comment'] . '</em><br />';
						$post_text .= 
							sprintf("<img src=\"%s\" title=\"\" alt=\"\" /> ", 
								$board_url . 'styles/' . $user_row['style_name'] . '/imageset/icon_topic_attach.gif') .
							sprintf("<b><a href=\"%s\">%s</a></b> (%s KiB)<br />",
			...
			if ($config['digests_block_images'] || $user_row['user_digest_block_images'])
			{
				$post_text = preg_replace('<img.*?\/>', '', $post_text);
			}
			...
			$template->assign_block_vars('forum.topic.post', array(
				...
				'SUBJECT'		=> ($is_html) ? sprintf("<a href=\"%sviewtopic.$phpEx?f=%s&amp;t=%s#p%s\">%s</a>%s", $board_url, $post_row['forum_id'], $post_row['topic_id'], $post_row['post_id'], html_entity_decode($post_row['post_subject']), "\n") : html_entity_decode($post_row['post_subject']),
