...
if ($user->data['user_id'] == ANONYMOUS)
	...
	switch ($_POST['cmd']) {
		case 'make_order_substitution': // Замена
			$sql = 'SELECT user_id
					FROM ' . ORDERS_TABLE . '
					WHERE order_id = ' . $_POST['order_id'];
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$user_id = $row['user_id']; // Id участника
			$sql = 'SELECT lot_cost,lot_orgrate
					FROM ' . LOTS_TABLE . '
					WHERE lot_id = ' . $_POST['lot_id'];
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$lot_cost = $row['lot_cost']; // Цена лота
			$org_fee = $row['lot_orgrate'];  // Орг сбор
			$sql = 'UPDATE ' . ORDERS_TABLE . '
					SET order_status=3
					WHERE order_id = ' . $_POST['order_id'];
			$db->sql_query($sql);
			
			$sql = 'SELECT purchase_id,catalog_course,catalog_valuta FROM phpbb_lots   LEFT OUTER JOIN phpbb_catalogs ON phpbb_lots.catalog_id = phpbb_catalogs.catalog_id WHERE phpbb_lots.lot_id ='.$_POST['lot_id'];
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$purchase_id= $row['purchase_id'];  // ID закупки
			$catalog_course= $row['catalog_course'];  // курс
			$catalog_valuta= $row['catalog_valuta'];  // валюта
				//(user_id,purchase_id,lot_id,order_properties,lot_cost,lot_orgrate,catalog_course,catalog_valuta )
			$sql = 'INSERT INTO '.ORDERS_TABLE.'
						(user_id,purchase_id,lot_id,order_properties,lot_cost,lot_orgrate,catalog_course,catalog_valuta)
				VALUES ('.$user_id.','.$purchase_id.','.$_POST['lot_id'].',\''.serialize($_POST['vars']).'\''.','.$lot_cost.','.$org_fee.','.$catalog_course.',\''.$catalog_valuta.'\')';
			$db->sql_query($sql);
		...
		case 'add_new_item':
			...
			$sql = 'INSERT 
				INTO '.LOTS_TABLE.' 
				(catalog_id,lot_name,lot_article,lot_properties, lot_orgrate )
				VALUES (
					'. $catalog_id .',
					"Новый лот","",
					\''.$row['prop'].'\',
...
//lot_id, catalog_id, lot_name, lot_cost, lot_orgrate, lot_properties, lot_description, lot_img, lot_hidden, lot_bundle
		...
		case 'user_out':
			...
			$sql = 'INSERT INTO phpbb_users_out (UserId) VALUES ( '.$user_id.')  ON DUPLICATE KEY UPDATE Date=NOW()';
			$db->sql_query($sql);
			//exit ($user_id);
			exit ("ok");
			break;
...
//error_log ('var '.$var[$k],0);
			...
			$result = $db->sql_query($sql);
			// Обновление цены и орга в заказах
			$sql = 'UPDATE '. ORDERS_TABLE .' SET
				lot_cost = "'. str_replace(',','.',$item['price']) .'",
				lot_orgrate = "'. $item['org_fee'] .'"
				WHERE lot_id = '. $item['id'].' AND purchase_id = '. $item['purchase_id'];
			//error_log ('sql '.$sql,0);
			$result = $db->sql_query($sql);
...
//error_log ('upd_cat_3:'.$sql,0);
			$result = $db->sql_query($sql);
			// получаем id закупки
			$sql="SELECT phpbb_catalogs.purchase_id FROM phpbb_catalogs WHERE phpbb_catalogs.catalog_id = ".$_POST['catalog_id'];
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$purchase_id= $row['purchase_id']; 			
			// Обновляем курс в заказах
			$sql="UPDATE phpbb_orders
					SET phpbb_orders.catalog_course= ". $_POST['catalog_course'] ."
					WHERE phpbb_orders.purchase_id = ".$purchase_id."
					AND phpbb_orders.lot_id IN (SELECT
					  phpbb_lots.lot_id
					FROM phpbb_lots
					WHERE phpbb_lots.catalog_id = ".$_POST['catalog_id'].")";
			$result = $db->sql_query($sql);

			
			break;
		case 'add_item_image': // Добавление картинки - закачкой файла
			...
			$sql="SELECT brand_id FROM phpbb_catalogs LEFT OUTER JOIN phpbb_purchases ON phpbb_catalogs.purchase_id = phpbb_purchases.purchase_id LEFT OUTER JOIN phpbb_reservs
					ON phpbb_purchases.reserv_id = phpbb_reservs.reserv_id RIGHT OUTER JOIN phpbb_lots ON phpbb_lots.catalog_id = phpbb_catalogs.catalog_id WHERE phpbb_lots.lot_id =".$id;
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
...
//error_log (var_export ($upload,true),0);
			
			$logo = $upload->form_upload($img);
			
			$logo->clean_filename('unique_ext');
//error_log (ini_get('upload_max_filesize'),0);
...
//			$arr = array ('status'=>"ok",'url'=>$url,'id'=>$count);
			$arr = ';ok;'.$url.';'.$count.';';
			$ex=str_replace('\\', '', json_encode($arr)); 
			exit ($ex);
			break;
		case 'add_item_image2': // Добавление картинки через ссылку
...
/*
	...
	*/		
			...
			$urls= unserialize($row['lot_img']); // Распаковываем перечень картинок в массив

			$url = $lot_img;
			$urls[]=$url; //Добавляем картинку в массив картинок
			$sql = 'UPDATE '. LOTS_TABLE .' SET
				lot_img=\''. serialize($urls).'\'
				WHERE lot_id='.$id;
			$result = $db->sql_query($sql);
...
//error_log ('$img='.$img,0);			
			...
			$sql="SELECT lot_id FROM phpbb_lots WHERE lot_img like(\"%".$img_."%\")";	
			$result = $db->sql_query($sql);	
//error_log ('$sql='.$sql,0);			

			if (!$result->num_rows) { //картинок нет - можно удалять
				$filename=$phpbb_root_path."images/lots/".$img_;
				$thumbname=$phpbb_root_path."images/lots/thumb/".$img_;
				if (file_exists($filename)) unlink($filename);
				if (file_exists($thumbname)) unlink($thumbname);
			}
		...
		case 'hide_lots':
			...
			$sql = 'UPDATE phpbb_catalogs
					...
					SET phpbb_lots.lot_hidden =1
					WHERE phpbb_catalogs.purchase_id = '.$purchase_id;
			$result = $db->sql_query($sql);
...
//error_log ('$sql='.$sql,0);
//error_log ('$result->num_rows'.$result->num_rows,0);

			while ($row1 = $db->sql_fetchrow($result1)){
				$lot_id =$row1['lot_id'];
//error_log ('$lot_id='.$lot_id,0);
				//----Удаляем лот---------------------------------------------
					// проверяем, есть ли заказы
					$sql='SELECT order_id FROM phpbb_orders WHERE phpbb_orders.lot_id = '.$lot_id;
					$result=$db->sql_query($sql);
...
//error_log ('$lot_id='.$lot_id,0);					
		...
		case 'purchases_upd':
			...
			if (isset($_POST['status_always_open']))	{
...
//			error_log($sql,0);
		...
		case 'add_to_order':
			...
			$sql = 'SELECT purchase_id FROM phpbb_lots   LEFT OUTER JOIN phpbb_catalogs ON phpbb_lots.catalog_id = phpbb_catalogs.catalog_id WHERE phpbb_lots.lot_id ='.$_POST['lot_id'];
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$purchase_id= $row['purchase_id']; 
			
			$sql = 'INSERT INTO '.ORDERS_TABLE.'
				(Create_Date,Purchase_id,user_id,lot_id,order_properties )
				VALUES (curdate(),'.$purchase_id.','.$user->data['user_id'].','.$_POST['lot_id'].',\''.serialize($arr).'\')';
			$db->sql_query($sql);
			$sql = 'SELECT *
			...
			if ($_POST['ec_id']) {
				...
				$arr['processed'][]=$v;
					...
					$sql1 = 'SELECT
							  ...
							  phpbb_orders.Purchase_id AS purchase_id1,
							...
							FROM phpbb_orders
							  ...
							  LEFT OUTER JOIN phpbb_user_purchases_count
								ON phpbb_orders.user_id = phpbb_user_purchases_count.user_id
								AND phpbb_catalogs.purchase_id = phpbb_user_purchases_count.purchase_id
...
//error_log ($sql1);		
					...
					if ($_POST['state']==3 or $_POST['state']<2) {
						$sql1 = 'SELECT
								...
								FROM phpbb_orders
								WHERE phpbb_orders.purchase_id = '. $purchase1 .' AND phpbb_orders.order_status IN (2, 4, 5, 6)';
//error_log ($sql1);						
						$result1 = $db->sql_query($sql1);
						$row1 = $db->sql_fetchrow($result1);
...
//error_log ($sql1);					
					...
					if (!$always_open) { //если  закупка не открыта постоянно проверяем на закрытие
						...
						$sql1='SELECT  COUNT( phpbb_orders.order_status) AS cnt
								  COUNT(phpbb_orders.order_status) AS cnt
								FROM phpbb_orders
								WHERE phpbb_orders.purchase_id = '.$purchase1.' AND phpbb_orders.order_status IN (0,1,2,4,5)';
//error_log ($sql1);						
						$result1 = $db->sql_query($sql1);
						$row1 = $db->sql_fetchrow($result1);
			...
			//  Если статус принято в ЕЦ, то рассылаем уведомление
			if ($_POST['state']==5) { 
				$xSQL = "SELECT
						  SubQuery.user_id,
						  SubQuery.username,
						...
						FROM (SELECT DISTINCT
							...
							SUBSTR(phpbb_purchases.purchase_url, INSTR(phpbb_purchases.purchase_url, 't=') + 2) AS theme
						  FROM phpbb_orders
							INNER JOIN phpbb_users
							  ON phpbb_orders.user_id = phpbb_users.user_id
							INNER JOIN phpbb_purchases
							  ON phpbb_orders.Purchase_id = phpbb_purchases.purchase_id
						  WHERE phpbb_orders.order_id IN (".implode(",",$_POST['orders']).")) SubQuery
						  INNER JOIN phpbb_topics
							ON SubQuery.theme = phpbb_topics.topic_id";	
				//error_log ('prihod '.$xSQL,0);	

				include_once($phpbb_root_path . 'includes/functions_messenger.' . $phpEx);
				// тут отправляется сообщение на почту 
				$messenger = new messenger();
				$result=$db->sql_query($xSQL);				
				while ($row = $db->sql_fetchrow($result)) {
					$messenger->template('toec', 'ru');
					$messenger->subject('Заказ доставлен');
					$messenger->to($row['user_email'],$row['user_realname'] ? $row['user_realname'] : $row['username']);
					$messenger->from('noreply@spmgn.ru');

					$messenger->assign_vars(array(
						'SUBJECT'		=> htmlspecialchars_decode('Заказ доставлен'),
						'AUTHOR_NAME'	=> htmlspecialchars_decode('Администрация сайта spmgn.ru'),
						'USERNAME'		=> htmlspecialchars_decode($row['user_realname'] ? $row['user_realname'] : $row['username']),
						'PUSHASE'		=> generate_board_url() .'/'. $row['purchase_url'],
						'PUSHASE_NAME'	=> htmlspecialchars_decode($row['topic_title']),
						'CART'			=> generate_board_url() .'/cart.php?p='. $row['purchase_id'],
					));
...
<br>			'.$a['usl'].'</div>';
		...
		case 'openpurchase':
			...
			$sql = 'SELECT
					...
					FROM phpbb_productcat
					  INNER JOIN phpbb_reservs
						ON phpbb_reservs.productcat_id = phpbb_productcat.productcat_id
					WHERE phpbb_reservs.reserv_id = '.$_POST['reserv_id'] . '
					AND phpbb_productcat.productcat_forum <> 0
					AND phpbb_reservs.status = 3';
...
//error_log ('$forum_id: '.$forum_id,0);
			...
			//	function add_post
			$sql='SELECT purchase_id FROM ' . PURCHASES_TABLE . ' WHERE reserv_id = ' . $_POST['reserv_id'];
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$id=$row['purchase_id'];
...
/*			
			...
			if ($row) {
				...
				if (!$joomla_material_id) { // если поле с id статьи пустое, то создаем статью
			...
			//			echo ($user_id);
				...
				// Добавляем категорию "Активные закупки"  CatID = 92
				$sql ='SELECT id FROM byfpd_k2_additional_categories WHERE itemID = '.$joomla_material_id.' AND catid = 92';
				$result=$db->sql_query($sql);
				$row = $db->sql_fetchrow($result);
				if (!$row) { //  Если категории нет, то вставляем
					$db->sql_query('INSERT INTO byfpd_k2_additional_categories (catid, itemID) VALUES (92, '.$joomla_material_id.')');
				}
				// Обновляем дату изменения материала
				$sql = 'UPDATE byfpd_k2_items SET modified= NOW() WHERE byfpd_k2_items.id = '.$joomla_material_id;
				//error_log ($sql,0);
				$db->sql_query($sql);
			}			
			*/
	...
	//		error_log ('t='.$t);
		...
		case 'set_kassa':
			...
			$sql ="UPDATE phpbb_orders 	
				  ...
				  WHERE phpbb_orders.order_status = \"5\" AND phpbb_orders.purchase_id =".$_POST['purshase_id']." AND phpbb_orders.user_id =".$_POST['user_id'];
			error_log ($sql,0);
			//echo $sql;
			$db->sql_query($sql);
			...
			if (!$always_open) { //если  закупка не открыта постоянно проверяем на закрытие
				...
				$sql1='SELECT  COUNT( phpbb_orders.order_status) AS cnt
						FROM phpbb_orders
						WHERE phpbb_orders.purchase_id = '.$_POST['purshase_id'].' AND phpbb_orders.order_status IN (0,1,2,4,5)';
				$result1 = $db->sql_query($sql1);
				$row1 = $db->sql_fetchrow($result1);
				$orders_count= $row1['cnt'];
				if (!$orders_count) { //если в закупке заказов не осталось - закрываем закупку
...
//			error_log ('User:'.$users,0);
			...
			$sql=' SELECT
			  phpbb_users.user_id,
			  IF(byfpd_user_profiles.profile_value IS FALSE OR byfpd_user_profiles.profile_value IS NULL, phpbb_users.user_phone, byfpd_user_profiles.profile_value) AS phone
			FROM phpbb_users
			...
			WHERE phpbb_users.user_id IN ( '.$_POST['users'].') GROUP BY phpbb_users.user_id';
			...
			if ($phones) {
			//error_log ('phones:'.implode (",",$phones),0);
			...
			//	error_log ('Sql insert'.$sql,0);
				$db->sql_query($sql);
				$row = $db->sql_fetchrow($db->sql_query("SELECT LAST_INSERT_ID() as id"));
				$log_id= $row['id']; 
			//	error_log ('$log_id:'.$log_id,0);
				$login = 'spmgn.ru';
				$pwd = 'JnRoust35105';
	...
	//			$exit =implode (",",$phones);
	//	error_log('Phones:'.implode (",",$phones));
			}

			//$exit = $_POST['text'];
			exit ($exit);
//			error_log('Sms status'.$exit,0);
			//exit ('ok');
			break;
		case 'change_purchase_admin_payment':
			...
			exit ('ok');
			break;
		case 'change_purchase_admin_payment_money':
			$sql = 'UPDATE '.PURCHASES_TABLE.'
				SET purchase_admin_payment_money="'.$_POST['text'].'"
				WHERE purchase_id='.$_POST['purchase_id'];
		...
		case 'set_delivery_p':
			...
			break;
		case 'copypurchase':
			$reserv_id = $_POST['reserv_id'];  //Старая бронь
//			error_log($reserv_id,0);
			...
			$sql = 'INSERT INTO '.RESERVS_TABLE.'
			 ...
			 ';
			 $db->sql_query($sql);
			$reserv_id2 = $db->sql_nextid(); //новая бронь
			...
			$sql = 'INSERT INTO '.PURCHASES_TABLE.'
			 ...
			 ';
			 $db->sql_query($sql);
			$purchase_id2 = $db->sql_nextid(); //новая закупка
			...
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$purchase_id = $row['purchase_id']; // старая закупка
			
			// выбираем каталоги в старой закупке
			$sql = 'select catalog_id from '.CATALOGS_TABLE.' WHERE purchase_id='.$purchase_id;
			$result = $db->sql_query($sql);

			while ($row = $db->sql_fetchrow($result)){
				$catalog_id = $row['catalog_id']; // Каталог из старой закупки
				...
				// Перебираем лоты в старом каталоге
				while ($row_l = $db->sql_fetchrow($result_lot)){
					$lot_id=$row_l['lot_id'];
					//Вставляем лоты в новый каталог
					$sql2 = 'INSERT INTO '.LOTS_TABLE.'
					 (catalog_id,lot_name,lot_article,lot_cost,lot_orgrate,lot_properties,lot_description,lot_img,lot_hidden,lot_bundle) 
						SELECT '.$catalog_id2.',lot_name,lot_article,lot_cost,lot_orgrate,lot_properties,lot_description,lot_img,lot_hidden,lot_bundle from '.LOTS_TABLE.' WHERE lot_id='.$lot_id.'
					 ';
					 $db->sql_query($sql2);
					 $lot_id2 = $db->sql_nextid();
					 // Если переносить новые заказы, то переносим
					 if ($nw==1){
						 $sql3 = 'UPDATE ' . ORDERS_TABLE . ' SET lot_id=' . $lot_id2 . ',purchase_id=' . $purchase_id2 . ' WHERE lot_id='. $lot_id .' AND order_status=0';
						 $db->sql_query($sql3);
...
//			error_log($reserv_id,0);
			...
			//Переносим каталоги из старой закупки в новую
			$sql= 'Update phpbb_catalogs Set purchase_id='.$purchase_id2.' WHERE purchase_id='.$purchase_id;
			$db->sql_query($sql);
			// Если переносить новые заказы, то переносим
			if ($nw==1){
				$sql = 'UPDATE ' . ORDERS_TABLE . ' SET purchase_id=' . $purchase_id2 . ' WHERE purchase_id='. $purchase_id .' AND order_status=0';
				$db->sql_query($sql);
			}			
			echo ';ok;';
			break;
