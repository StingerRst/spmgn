		...
		*/
		switch ($mode)
		{
			...
			case 'features':
				$display_vars = array(
					...
					'vars'	=> array(
						...
						'sms_activation'		=> array('lang' => 'SMS_ACTIVATION',		'validate' => 'bool',	'type' => 'radio:yes_no', 'explain' => false),
			...
			case 'digest_edit_subscribers':
				...
				$subscribe = request_var('subscribe', 's');
				...
				switch ($subscribe)
				{
					...
					break;
					case 's':
						$subscribe_sql = "user_digest_type <> 'NONE' AND user_digest_send_hour_gmt >= 0 AND user_digest_send_hour_gmt <= 24 AND";
				...
				$last_sent_selected = $has_unsubscribed_selected = $username_selected = $frequency_selected = $format_selected = $hour_selected = $type_selected = $lastvisit_selected = '';
				switch ($sortby)
				{
					case 'f':
						$sort_by_sql = 'user_digest_type, lower(username)';
						$frequency_selected = ' selected="selected"';
					break;
					case 's':
						$sort_by_sql = 'user_digest_format, lower(username)';
						$format_selected = ' selected="selected"';
					break;
					case 'h':
						$sort_by_sql = 'send_hour_board, lower(username)';
						$hour_selected = ' selected="selected"';
					break;
					case 'l':
						$sort_by_sql = 'user_lastvisit, lower(username)';
						$lastvisit_selected = ' selected="selected"';
					break;
					case 'b':
						$sort_by_sql = 'user_digest_has_unsubscribed, lower(username)';
						$has_unsubscribed_selected = ' selected="selected"';
					break;
					case 't':
						$sort_by_sql = 'user_digest_last_sent, lower(username)';
						$last_sent_selected = ' selected="selected"';
					break;
					case 'u':
					default:
						$sort_by_sql = 'lower(username)';
				...
				$sql = 'SELECT *, CASE
					...
					FROM ' . USERS_TABLE . "
					WHERE $subscribe_sql $member_sql user_type <> " . USER_IGNORE . "
					ORDER BY $sort_by_sql $order_by_sql";
				...
				while ($row = $db->sql_fetchrow($result))
				{
					...
					$template->assign_block_vars('digest_edit_subscribers', array(
						...
						'USER_DIGEST_LAST_SENT'		=> ($row['user_digest_last_sent'] == 0) ? $user->lang['DIGEST_NO_DIGESTS_SENT'] : date($config['default_dateformat'], $row['user_digest_last_sent'] + (3600 * $config['board_timezone']) + ($config['board_dst'] ? 3600 : 0)),
						'USER_DIGEST_SEND_HOUR'		=> $send_hour_board,
						'USER_DIGEST_TYPE'			=> $digest_type,
						'USER_ID'					=> $row['user_id'],
						'USER_LAST_VISIT'			=> ($row['user_lastvisit'] == 0) ? $user->lang['DIGEST_NEVER_VISITED'] : date($config['default_dateformat'], $row['user_lastvisit'] + (3600 * $config['board_timezone']) + ($config['board_dst'] ? 3600 : 0)),
			...
			case 'digest_balance_load':
				...
				for($i=0;$i<24;$i++)
				{
					...
					// Convert board time to GMT, to the hour if necessary to accommodate strange timezones
					$hour_gmt = floor($i - $config['board_timezone'] - $config['board_dst']);
					if ($hour_gmt < 0)
					{
					...
					// If there are digest counts for this hour, show it, otherwise show zero (no digests for this hour)
					$hour_count = (isset($rowset[$rowset_ptr]['hour']) && ($rowset[$rowset_ptr]['hour'] == $hour_gmt)) ? $rowset[$rowset_ptr]['hour_count'] : 0; 
					$template->assign_block_vars('digest_balance_load', array(
						'HOUR'					=> $i,
						'HOUR_COUNT'			=> $hour_count,
					));
					
					if (isset($rowset[$rowset_ptr]['hour']) && ($rowset[$rowset_ptr]['hour'] == $i))
					{
						$rowset_ptr++;
					}
		...
		if ($submit)
		{
			...
			if ($mode == 'digest_edit_subscribers')
			{
				...
				foreach ($_POST as $name => $value)
				{
					...
					if (substr(htmlspecialchars($name),0,5) == 'user-')
					{
						...
						if (substr(htmlspecialchars($name),5,2) == 'u-')
						{
							// unsubscribe this user
							$sql2 = 'UPDATE ' . USERS_TABLE . "  
								SET user_digest_type = 'NONE'
								WHERE user_id = " . substr(htmlspecialchars($name),7);
			...
			if ($mode == 'digest_balance_load')
			{
				...
				// Average number of subscribers per hour. We need to assume at least one subscriber per hour to avoid resetting user's preferred
				// digest time unnecessarily. If the average is 3 per hour, the first 3 already subscribed will not have their digest arrival time
				// changed.
				...
				$sql = 'SELECT user_id
					...
					WHERE user_digest_type <> 'NONE' AND user_type <> " . USER_IGNORE . '
					GROUP BY user_digest_send_hour_gmt
					HAVING count(user_digest_send_hour_gmt ) > ' . $avg_subscribers_per_hour . '
					ORDER BY 1';
				...
				trigger_error(sprintf($user->lang['DIGEST_REBALANCED'], sizeof($rowset)) . $custom_message . adm_back_link($this->u_action));
		...
		$template->assign_vars(array(
			...
			'S_DIGESTS'				=> ($mode == 'digest_general' || $mode == 'digest_user_defaults' || $mode == 'digest_edit_subscribers' || $mode == 'digest_balance_load') ? true : false,
