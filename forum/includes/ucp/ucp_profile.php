...
class ucp_profile
{
	...
	function main($id, $mode)
	{
			...
			case 'profile_info':
				...
				$data = array(
					...
					'interests'		=> utf8_normalize_nfc(request_var('interests', $user->data['user_interests'], true)),
										//sms
					'phone'			=> request_var('phone', $user->data['user_phone']),
					//end sms
					'realname'				=> utf8_normalize_nfc(request_var('realname', $user->data['user_realname'], true)),
					'pasportdata_number' 	=> utf8_normalize_nfc(request_var('pasportdata_number', $user->data['user_pasportdata_number'], true)),
					...
					'pasportdata_addres'	=> utf8_normalize_nfc(request_var('pasportdata_addres', $user->data['user_pasportdata_addres'], true)),
					'company_name'			=> utf8_normalize_nfc(request_var('company_name', $user->data['user_company_name'], true)),
					'company_addres'		=> utf8_normalize_nfc(request_var('company_addres', $user->data['user_company_addres'], true)),
					'company_inn'			=> utf8_normalize_nfc(request_var('company_inn', $user->data['user_company_inn'], true)),
					'company_regdate'		=> utf8_normalize_nfc(request_var('company_regdate', $user->data['user_company_regdate'], true)),
				);
				
				//explode date of paspartdate on days, month, years
				list($data['pasportdata_issueddate_day'], $data['pasportdata_issueddate_month'], $data['pasportdata_issueddate_year']) = explode('-', $user->data['user_pasportdata_issueddate']);
				$data['pasportdata_issueddate_day'] 	= request_var('pasportdata_issueddate_day', $data['pasportdata_issueddate_day']);
				$data['pasportdata_issueddate_month'] 	= request_var('pasportdata_issueddate_month', $data['pasportdata_issueddate_month']);
				$data['pasportdata_issueddate_year'] 	= request_var('pasportdata_issueddate_year', $data['pasportdata_issueddate_year']);
				$data['pasportdata_issueddate'] 		= sprintf('%2d-%2d-%4d', $data['pasportdata_issueddate_day'], $data['pasportdata_issueddate_month'], $data['pasportdata_issueddate_year']);
				
				//explode date of registraton company on days, month, years
				list($data['company_regdate_day'], $data['company_regdate_month'], $data['company_regdate_year']) = explode('-', $user->data['user_company_regdate']);
				$data['company_regdate_day'] 	= request_var('company_regdate_day', $data['company_regdate_day']);
				$data['company_regdate_month'] 	= request_var('company_regdate_month', $data['company_regdate_month']);
				$data['company_regdate_year'] 	= request_var('company_regdate_year', $data['company_regdate_year']);
				$data['company_regdate'] 		= sprintf('%2d-%2d-%4d', $data['company_regdate_day'], $data['company_regdate_month'], $data['company_regdate_year']);
				...
				if ($submit)
				{
					$validate_array = array(
						...
						'phone'			=> array(
							array('string', true, 5, 15)),
							//array('match', true, '#^\+[0-9]+$#i')),
						//end sms
						'pasportdata_number'    => array(
							array('string', true, 3, 30),
							array('match', true, '#^[ 0-9]+$#i')),
					);
					...
					if (!sizeof($error))
					{
						...
						$sql_ary = array(
							...
							'user_pasportdata_number' 		=> $data['pasportdata_number'],
							'user_pasportdata_issuedby' 	=> $data['pasportdata_issuedby'],
							'user_pasportdata_issueddate' 	=> $data['pasportdata_issueddate'],
							'user_pasportdata_addres'		=> $data['pasportdata_addres'],
							'user_company_name' 			=> $data['company_name'],
							'user_company_addres' 			=> $data['company_addres'],
							'user_company_inn' 				=> $data['company_inn'],
							'user_company_regdate' 			=> $data['company_regdate'],
						);
        ...
        //Create options for pasportdata_date
          $pasportdata_issueddate_day_options = '<option value="0"' . ((!$data['pasportdata_issueddate_day']) ? ' selected="selected"' : '') . '>--</option>';
					for ($i = 1; $i < 32; $i++)
					{
						$selected = ($i == $data['pasportdata_issueddate_day']) ? ' selected="selected"' : '';
						$pasportdata_issueddate_day_options .= "<option value=\"$i\"$selected>$i</option>";
					}

					$pasportdata_issueddate_month_options = '<option value="0"' . ((!$data['pasportdata_issueddate_month']) ? ' selected="selected"' : '') . '>--</option>';
					for ($i = 1; $i < 13; $i++)
					{
						$selected = ($i == $data['pasportdata_issueddate_month']) ? ' selected="selected"' : '';
						$pasportdata_issueddate_month_options .= "<option value=\"$i\"$selected>$i</option>";
					}
					$pasportdata_issueddate_year_options = '';

					$now = getdate();
					$pasportdata_issueddate_year_options = '<option value="0"' . ((!$data['pasportdata_issueddate_year']) ? ' selected="selected"' : '') . '>--</option>';
					for ($i = $now['year'] - 100; $i <= $now['year']; $i++)
					{
						$selected = ($i == $data['pasportdata_issueddate_year']) ? ' selected="selected"' : '';
						$pasportdata_issueddate_year_options .= "<option value=\"$i\"$selected>$i</option>";
					}
					unset($now);

					$template->assign_vars(array(
						'PASPORTDATA_ISSUEDDATE_DAY_OPTIONS'	=> $pasportdata_issueddate_day_options,
						'PASPORTDATA_ISSUEDDATE_MONTH_OPTIONS'	=> $pasportdata_issueddate_month_options,
						'PASPORTDATA_ISSUEDDATE_YEAR_OPTIONS'	=> $pasportdata_issueddate_year_options,
					));
        
        //Create options for pasportdata_date
          $company_regdate_day_options = '<option value="0"' . ((!$data['company_regdate_day']) ? ' selected="selected"' : '') . '>--</option>';
					for ($i = 1; $i < 32; $i++)
					{
						$selected = ($i == $data['company_regdate_day']) ? ' selected="selected"' : '';
						$company_regdate_day_options .= "<option value=\"$i\"$selected>$i</option>";
					}

					$company_regdate_month_options = '<option value="0"' . ((!$data['company_regdate_month']) ? ' selected="selected"' : '') . '>--</option>';
					for ($i = 1; $i < 13; $i++)
					{
						$selected = ($i == $data['company_regdate_month']) ? ' selected="selected"' : '';
						$company_regdate_month_options .= "<option value=\"$i\"$selected>$i</option>";
					}
					$company_regdate_year_options = '';

					$now = getdate();
					$company_regdate_year_options = '<option value="0"' . ((!$data['company_regdate_year']) ? ' selected="selected"' : '') . '>--</option>';
					for ($i = $now['year'] - 100; $i <= $now['year']; $i++)
					{
						$selected = ($i == $data['company_regdate_year']) ? ' selected="selected"' : '';
						$company_regdate_year_options .= "<option value=\"$i\"$selected>$i</option>";
					}
					unset($now);

					$template->assign_vars(array(
						'COMPANY_REGDATE_DAY_OPTIONS'	=> $company_regdate_day_options,
						'COMPANY_REGDATE_MONTH_OPTIONS'	=> $company_regdate_month_options,
						'COMPANY_REGDATE_YEAR_OPTIONS'	=> $company_regdate_year_options,
					));
				...
				$template->assign_vars(array(
					...
					'DBPHONE'	=> (!empty($user_info['user_phone'])) ? $user_info['user_phone'] : '',
					'PHONE'		=> $data['phone'],
					'PASPORTDATA_NUMBER' => $data['pasportdata_number'],
					'PASPORTDATA_ISSUEDBY' => $data['pasportdata_issuedby'],
					'PASPORTDATA_ISSUEDDATE' => $data['pasportdata_issueddate'],
					'PASPORTDATA_ADDRES'    => $data['pasportdata_addres'],
