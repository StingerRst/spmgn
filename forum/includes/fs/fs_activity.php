...
class fs_activity
{
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'forums':
				...
				if ($config['num_topics'])
				{
					...
					{					
						...
						'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_forum['f_id'] . '" title="' . $current_forum['f_name'] . '">' . $current_forum['f_name'] . '</a>',
						'COUNT'				=> $current_forum['count'],
						'PCT'						=> number_format($current_forum['count'] / $config['num_topics'] * 100, 3),
				...
				if ($config['num_posts'])
				{
					foreach ($top_forums_by_posts as $current_forum)
					{
						...
						'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_forum['f_id'] . '" title="' . $current_forum['f_name'] . '">' . $current_forum['f_name'] . '</a>',
						'COUNT'				=> $current_forum['count'],
						'PCT'						=> number_format($current_forum['count'] / $config['num_posts'] * 100, 3),
						'BARWIDTH'					=> number_format($current_forum['count'] / $max_count * 100, 1),
						));
					}
				}
				...
				if ($total_polls)
				{
					foreach ($top_forums_by_polls as $current_forum)
					{
						...
						'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_forum['f_id'] . '" title="' . $current_forum['f_name'] . '">' . $current_forum['f_name'] . '</a>',
						'COUNT'				=> $current_forum['count'],
						'PCT'						=> number_format($current_forum['count'] / $total_polls * 100, 3),
						'BARWIDTH'					=> number_format($current_forum['count'] / $max_count * 100, 1),
						));
					}
				}
				...
				if ($top_forums_by_sticky) //get total sticky count only if there are any forums with sticky topics returned, hence save 1 query otherwise
				{
					...
					foreach ($top_forums_by_sticky as $current_forum)
					{
						$template->assign_block_vars('top_by_sticky_row', array(
								...
								'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_forum['f_id'] . '" title="' . $current_forum['f_name'] . '">' . $current_forum['f_name'] . '</a>',
								'COUNT'				=> $current_forum['count'],
								'PCT'						=> number_format($current_forum['count'] / $total_sticky * 100, 3),
								'BARWIDTH'					=> number_format($current_forum['count'] / $max_count * 100, 1),
								));
					}
				}
				...
				if ($total_participation)
				{
					foreach ($top_forums_by_participation as $current_forum)
					{
						...
						'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_forum['f_id'] . '" title="' . $current_forum['f_name'] . '">' . $current_forum['f_name'] . '</a>',
						'COUNT'				=> $current_forum['count'],
						'PCT'						=> number_format($current_forum['count'] / $total_participation * 100, 3),
						'BARWIDTH'					=> number_format($current_forum['count'] / $max_count * 100, 1),
						));
					}
				}
			...
			case 'topics':
				...
				if ($total_posts)
				{
					...
					{
						$template->assign_block_vars('top_by_posts_row', array(
						'U_TOPIC'					=> "<a href=\"{$phpbb_root_path}viewtopic.$phpEx?f=" . $current_topic['f_id'] . '&amp;t=' . $current_topic['t_id'] . '" title="' . $current_title['t_title'] . '">' . $current_topic['t_title'] . '</a>',						
						'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_topic['f_id'] . '" title="' . $current_topic['f_name'] . '">' . $current_topic['f_name'] . '</a>',
						'COUNT'				=> $current_topic['count'],
						'PCT'						=> number_format($current_topic['count'] / $total_posts * 100, 3),
				...
				if ($total_views)
				{
					foreach ($top_topics_by_views as $current_topic)
					{
						$template->assign_block_vars('top_by_views_row', array(
							...
							'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_topic['f_id'] . '" title="' . $current_topic['f_name'] . '">' . $current_topic['f_name'] . '</a>',
							'COUNT'				=> $current_topic['count'],
							'PCT'						=> number_format($current_topic['count'] / $total_views * 100, 3),
							'BARWIDTH'					=> number_format($current_topic['count'] / $max_count * 100, 1),
						));
					}
				}
				...
				if ($total_participation)
				{
					foreach ($top_topics_by_participation as $current_topic)
					{
						...
						'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_topic['f_id'] . '" title="' . $current_topic['f_name'] . '">' . $current_topic['f_name'] . '</a>',
						'COUNT'				=> $current_topic['count'],
						'PCT'						=> number_format($current_topic['count'] / $total_participation * 100, 3),
						'BARWIDTH'					=> number_format($current_topic['count'] / $max_count * 100, 1),
						));
					}
				}
				...
				if ($total_attachments)
				{
					...
					{					
						...
						'U_FORUM'					=> "<a href=\"{$phpbb_root_path}viewforum.$phpEx?f=" . $current_topic['f_id'] . '" title="' . $current_topic['f_name'] . '">' . $current_topic['f_name'] . '</a>',
						'COUNT'				=> $current_topic['count'],
						'PCT'						=> number_format($current_topic['count'] / $total_attachments * 100, 3),
						'BARWIDTH'					=> number_format($current_topic['count'] / $max_count * 100, 1),
						));
					}
				}
				...
				$template->assign_vars(array(
					...
					'S_TOP_TOPICS_BY_VIEWS'		=> ($top_topics_by_views) ? true : false,
			...
			case 'users':
				...
				$member_counts = get_total_members($total_members, $groups_data);				
				...
				if (sizeof($ranks_data))
				{
					...
					$user_ranks_count = get_users_ranks_count('non-special'); //this returns all the post counts of the users
					...
					if (array_sum($ranks_counts))
					{
						...
						$temp_counter = count($ranks_data) - 1;
				...
				if ($top_users_by_posts)
				{
					...
					{						
						$template->assign_block_vars('top_by_posts_row', array(
							...
							'PCT'						=> number_format($current_user['count'] / $config['num_posts'] * 100, 3),
				...
				if ($top_users_by_topics)
				{
					...
					{						
						$template->assign_block_vars('top_by_topics_row', array(
							'U_USER'					=> get_username_string('full', $current_user['u_id'], $current_user['username'], $current_user['u_colour']),							
							'COUNT'				=> $current_user['count'],
							'PCT'						=> number_format($current_user['count'] / $config['num_topics'] * 100, 3),
							'BARWIDTH'					=> number_format($current_user['count'] / $max_count * 100, 1),
						));
					}
				}
				...
				if ($num_recent_posts)
				{
					...
					if ($top_users_by_recent_posts)
					{
						...
						{						
							...
							'U_USER'					=> get_username_string('full', $current_user['u_id'], $current_user['username'], $current_user['u_colour']),							
							'COUNT'				=> $current_user['count'],
							'PCT'						=> number_format($current_user['count'] / $num_recent_posts * 100, 3),
							'BARWIDTH'					=> number_format($current_user['count'] / $max_count * 100, 1),
							));
						}
					}
				...
				$template->assign_vars(array(
					...
					'U_WHO_IS_ONLINE'				=> '<a href="' . $phpbb_root_path . 'viewonline.' . $phpEx . '">' . $user->lang['WHO_IS_ONLINE'] . '</a> (' . sprintf($user->lang['WHO_IS_ONLINE_EXPLAIN'], $config['load_online_time']) . ')',
		...
		$template->assign_vars(array(
			'L_TITLE'	=> $user->lang['FS_ACTIVITY_' . strtoupper($mode)],			
			'S_FS_ACTION'		=> $this->u_action,
			'AS_ON'				=> sprintf($user->lang['AS_ON'], $user->format_date(time())),
		));
		
		$this->tpl_name = 'fs_activity_' . $mode;
