	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'daily':
				...
				if ($totals['topics'])
				{
					...
					foreach ($daily_data as $day => $data)
					{
						$template->assign_block_vars('periodic_topics_row', array(
							'TIME_ELEMENT'				=> date('d F Y', mktime(0, 0, 0, $selected_month[0], $day, $selected_month[1])),
							'COUNT'						=> $data['topics'],
				...
				$month_select_box = make_select_box($month_options, $selected_month[0] . '-' . $selected_month[1], 'selected_month', $user->lang['SHOW_STATS_FOR_MONTH'], $user->lang['GO'], $this->u_action);
				...
				$template->assign_vars(array(
					...
					'STATS_MONTH_EXPLAIN'		=> sprintf($user->lang['STATS_MONTH_EXPLAIN'], $start_time['month'] . ' ' . $start_time['year']),
			...
			case 'monthly':
				...
				$template->assign_vars(array(
					...
					'STATS_YEAR_EXPLAIN'		=> sprintf($user->lang['STATS_YEAR_EXPLAIN'], $start_time['year'] != $current_time['year'] ? ($show_all ? $start_time['year'] . ' - ' . $current_time['year'] : $current_time['year']) : $current_time['year']),
					'MONTH_SELECT_BOX'			=> $year_select_box,
				));
			break;
			
			case 'hourly':
				$current_time = getdate(time()); //store the time, which will be used uniformly all throughout this function
				...
				if ($request_month != 'all')
				{
					...
					if ($request_day != 'all')
					{
						...
						else if ($request_month[0] == $board_starttime['mon'] && $request_month[1] == $current_time['year'] && $request_day < $current_time['mday']) //if its the board start month then if its before the start day, set it to start date
						{
				...
				if ($total_topics)
				{
					...
					for ($i = 0; $i < 24; $i++)
					{
						...
						'PCT'				=> number_format($all_stats['topics'][$i] / $total_topics * 100, 3),
				...
				if ($total_posts)
				{
					...
					for ($i = 0; $i < 24; $i++)
					{
						...
						'PCT'				=> number_format($all_stats['posts'][$i] / $total_posts * 100, 3),
				...
				if ($total_user_regs)
				{
					...
					for ($i = 0; $i < 24; $i++)
					{
						...
						'PCT'				=> number_format($all_stats['user_reg'][$i] / $total_user_regs * 100, 3),
				...
				{
					$month_select_box .= '<option value="' . date('n-Y', $temp_time) . '"' . ((date('n-Y', $temp_time) == $selected_month) ? ' selected="selected"' : '') . '>' . date('F Y', $temp_time) . '</option>';
					$count_months++;
				}
				...
				$template->assign_vars(array(
					...
					'SELECT_BOX'				=> '<form method="post" action="' . $this->u_action . '"><label for="sel_day">' . $user->lang['SELECT_TIME_PERIOD'] . ': </label>' . $day_select_box . ' ' . $month_select_box . ' <input type="submit" class="button2" value="' . ucfirst($user->lang['GO']) . '" /></form>',
					...
					'PCT_TOPICS'				=> number_format($total_topics / $config['num_topics'] * 100, 5),
					'PCT_POSTS'				=> number_format($total_posts / $config['num_posts'] * 100, 5),
					'PCT_USER_REGS'				=> number_format($total_user_regs / $config['num_users'] * 100, 5),
					'HOURLY_STATS_EXPLAIN'		=> sprintf($user->lang['HOURLY_STATS_EXPLAIN'], (($show_all_month) ? $board_starttime['month'] . ' ' . $board_starttime['year'] . ' - ' . $current_time['month'] . ' ' . $current_time['year'] : ((!$show_all_day) ? str_pad($request_day, 2, '0', STR_PAD_LEFT) . ' ' : '') . date('F Y', mktime(0, 0, 0, $request_month[0], 1, $request_month[1])))),
				));
			break;
		...
		$template->assign_vars(array(
			'L_TITLE'	=> $user->lang['FS_PERIODIC_' . strtoupper($mode)],			
			'S_FS_ACTION'		=> $this->u_action,
			'AS_ON'				=> sprintf($user->lang['AS_ON'], $user->format_date($current_time[0])),
		));
		
		$this->tpl_name = 'fs_periodic_' . $mode;
