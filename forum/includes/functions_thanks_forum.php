...
{
	global $max_forum_thanks, $config, $phpbb_root_path, $template;
	$template->assign_block_vars('forumrow.reput', array(
		'FORUM_REPUT'				=> (get_thanks_forum_number($forum_id) != 0) ? round(get_thanks_forum_number($forum_id) / ($max_forum_thanks / 100), $config['thanks_number_digits']).'%' : '',
...
// forum thanks number
function get_thanks_forum_number($forum_id)
{
	global $db;
	...
	$forum_thanks = (int) $db->sql_fetchfield('forum_thanks');
