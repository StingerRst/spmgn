	...
	$purchase_id = request_var('p', 0);;
  	$sql = 'SELECT 
			...
			FROM ' . PURCHASES_TABLE . '
      		...
      		WHERE ' . RESERVS_TABLE . '.user_id = '.$u_id.'
	...
	if ($row2['Keep']==0){
		$sql='SELECT ROUND(COUNT(SubQuery.user_id) * 5 + SUM(SubQuery.CostS) / 50) AS Itogo
			FROM (SELECT phpbb_orders.user_id,
				SUM(phpbb_orders.lot_cost * phpbb_orders.catalog_course) AS CostS
			  FROM phpbb_reservs
...
//echo ($sql);
	...
	$sql = 'SELECT
		...
		$sql .= ' AND phpbb_lots.lot_id IS NOT NULL';
...
//	echo $sql;
   	...
   	while ($row = $db->sql_fetchrow($result)) {
		...
		$orders[$row['order_id']]['lot_name'] = addslashes($row['lot_name']);
		$orders[$row['order_id']]['catalog_id'] = $row['catalog_id'];
		$orders[$row['order_id']]['catalog_name'] = addslashes($row['catalog_name']);
...
//echo $sql;			
	$result=$db->sql_query($sql);
   	while ($row = $db->sql_fetchrow($result)) {
		...
		$catalogs[$row['catalog_id']]['name']	= addslashes($row['catalog_name']);
		if ($row['catalog_id']) {
			$sqll = 'SELECT *
				FROM ' . LOTS_TABLE . '
	...
	//echo $sql;			
		...
		while ($row2 = $db->sql_fetchrow($result2)) {
			$lots[$row2['lot_id']]['valuta'] 		= addslashes($row2['catalog_valuta']);
			...
			$lots[$row2['lot_id']]['lot_article']	= addslashes($row2['lot_article'])?('('.$row2['lot_article'].') '):'';
			$lots[$row2['lot_id']]['lot_name'] 		= $row2['lot_name'];
	...
	if (is_array($lots)){
		...
		foreach($lots as $k=>$v){
			...
			$lot.='
				'.$k.':{
					"id":'.$v['lot_id'].',
					"article":"'.addslashes($v['lot_article']).'",
					"name":"'.addslashes($v['lot_name']).'",
					"catalog_id":'.$v['catalog_id'].',
					"price":'.$v['price'].',
					"price_u":'.$v['price_u'].',
					"valuta":"'.addslashes($v['valuta']).'",
					"vars":{';
						$f3=0;
						if (is_array($v['vars']))
						foreach($v['vars'] as $vk=>$vv){
