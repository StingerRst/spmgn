...
*
* @package phpBB3
* @version $Id: prime_links.php,v 1.2.4 2008/10/14 00:16:00 primehalo Exp $
	...
	*/
	class prime_links
	{
		...
		*/
		...
		{
			$this->top_level_domain_name = TOP_LEVEL_DOMAIN_NAME ? str_replace('.', '\\.', TOP_LEVEL_DOMAIN_NAME) : '\.(?:aero|biz|com|coop|info|jobs|museum|name|net|org|pro|travel|gov|edu|mil|int)';
		...
		*/
		function remove_subdomains($url, $remove = true)
		{
			...
			if ($remove !== false && strpos($url, '//') !== false)
			{
				...
				else
				{
					...
					$stripped_url = preg_replace('#^(http|https)://[^/]+\.([a-z0-9-]+' . $this->top_level_domain_name . ')#i', '$1://$2', $url);
					if ($stripped_url == $url)
					{
		...
		/**
		* Determains if the URL contains a domain. 
