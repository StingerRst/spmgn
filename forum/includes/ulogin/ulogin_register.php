...
//	error_log ($config['require_activation']."\n",3,'/var/www/html/spmgn.ru/forum/logs/debug.log');

		// Если активация отключена, то должны выйти но унас регистрация через jfusion, так что не выходим	
		if ($config['require_activation'] == USER_ACTIVATION_DISABLE)
		{
			//trigger_error('UCP_REGISTER_DISABLE');
		}
		
		if (!$user_id = $uLogin->auth()) // Если пользователя нет , то регистрируем
		{	// Если регистрация отключена, то выходим
			if ($config['require_activation'] == USER_ACTIVATION_DISABLE) 
				{
					trigger_error('UCP_REGISTER_DISABLE');
				}
			$user_id = $uLogin->register(); // Регистрирунм
		}
