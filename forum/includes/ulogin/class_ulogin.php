...
class uLogin
{
   ...
   private function __get_response($url = ""){
	 ...
	 * 
	 * @access 	public
	 * @return 	bool				if user authorized return userid, else return false
	 */
	public function auth()
	{
	//error_log ("auth\n",3,'/var/www/html/spmgn.ru/forum/logs/debug.log');
		//Если пользователя нет в ulogin_users , то выходим
		if (!$user = $this->__get_first("SELECT
										  byfpd_ulogin_users.user_id,
										  byfpd_ulogin_users.identity,
										  byfpd_ulogin_users.network,
										  byfpd_jfusion_users_plugin.userid
										FROM byfpd_ulogin_users
										  LEFT OUTER JOIN byfpd_jfusion_users_plugin
											ON byfpd_ulogin_users.user_id = byfpd_jfusion_users_plugin.id
										WHERE byfpd_ulogin_users.identity =  '" . $this->db->sql_escape($this->user['identity']) . "'"))
		{
			return false;
		}
		//error_log ("auth2\n",3,'/var/www/html/spmgn.ru/forum/logs/debug.log');
		// Если пользователь есть в  ulogin_users но его нет в phpbb_users, то удаляем запись из ulogin_users и выходим
		if (!$this->__get_first("SELECT * FROM `" . USERS_TABLE . "` WHERE user_id = " . $user['userid']))
		{
			$this->db->sql_query("DELETE FROM byfpd_ulogin_users WHERE identity = '" . $this->db->sql_escape($this->user['identity']) . "'");
		
			return false;
		}
		// пользователь есть везде
		return $user['userid'];
	}
	...
	public function register()
	{
		...
		// to do: Получаем Joomla user id, дабавляем запись в ulogin
