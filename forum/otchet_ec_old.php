...
$sql ="SELECT phpbb_purchases.purchase_id, phpbb_purchases.purchase_name, phpbb_users.user_id, phpbb_users.username, phpbb_purchases.purchase_admin_payment\n"
    . "     , round(sum(SubQuery.`Сумма`) / 100 * 2, 1) AS '2percent',  count(SubQuery.user_id) * 5 AS 'v_ec'\n"
    . "     , round(sum(SubQuery.`Сумма`) / 100 * 2 +  count(SubQuery.user_id) * 5, 1) AS 'itogo'\n"
    . "     , phpbb_purchases.purchase_admin_money AS 'zaplacheno',min(SubQuery.ToEcMin) AS 'ToEc'\n"
    . "FROM\n"
    . "  phpbb_purchases\n"
    . "RIGHT OUTER JOIN (\n"
    . "SELECT phpbb_catalogs.purchase_id\n"
    . "     , sum(phpbb_lots.lot_cost *catalog_course ) AS `Сумма`\n"
    . "     , phpbb_orders.user_id\n"
    . "     , min(phpbb_orders.To_EC_Date) AS `ToEcMin` \n"
    . "FROM\n"
    . "  phpbb_catalogs\n"
    ...
    . "WHERE\n"
    . "  phpbb_orders.order_status > 3 AND YEAR (phpbb_orders.To_EC_Date) > 2013 \n"
    . "GROUP BY\n"
...
//    if ($hide) $sql .="  and isnull(phpbb_purchases.purchase_admin_money)";
    if (!$hide) $sql .=" and (isnull(phpbb_purchases.purchase_admin_money) or (phpbb_purchases.purchase_admin_money) <1)";
	$sql .= " GROUP BY\n"
    ...
    . "ORDER BY\n"
    . "  ToEc DESC ,Ucase(phpbb_users.username) ";
