...
page_header('Cart');
		...
		$sql = 'SELECT
				...
				WHERE phpbb_user_purchases_count.user_id = '.$user->data['user_id'];
		...
		$sql = 'SELECT
				...
				WHERE phpbb_org_purchases_count.user_id = '.$user->data['user_id'];
		$result1=$db->sql_query($sql);
		$row1 = $db->sql_fetchrow($result1);
		$rating2 = ($row1['rating2'])?$row1['rating2']:0;	

		$template->assign_vars(array(
 		  		'USER_INFO'  		=> $user->data['user_phone'],
		...
		JOIN ' . USERS_TABLE . ' ON ' . RESERVS_TABLE . '.user_id = ' . USERS_TABLE . '.user_id
		WHERE ' . ORDERS_TABLE . '.user_id = '.$user->data['user_id'].'
