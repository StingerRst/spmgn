...
switch( $mode )
{
	case "start":
		...
		$db->sql_query('UPDATE ' . RESERVS_TABLE . '
			SET status = 2
			WHERE reserv_id = ' . $rid);
		redirect(append_sid("{$phpbb_root_path}org.$phpEx",'i=1'));
	break;
	default:
		$template_body = "org.html";
	

}

$tabs = '<ul>
	<li'.(($id==0)?' class="activetab"':'').'><a href="'.append_sid("{$phpbb_root_path}org.$phpEx",'i=0').'"><span>Новые закупки и брони</span></a></li>
	<li'.(($id==1)?' class="activetab"':'').'><a href="'.append_sid("{$phpbb_root_path}org.$phpEx",'i=1').'"><span>Текущие закупки</span></a></li>
