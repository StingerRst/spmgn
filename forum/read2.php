...
$sql ='SELECT
		...
		FROM phpbb_users
		  RIGHT OUTER JOIN phpbb_privmsgs
			ON phpbb_users.user_id = phpbb_privmsgs.author_id
		  RIGHT OUTER JOIN phpbb_privmsgs_to
		...
		WHERE phpbb_users_1.username <> phpbb_users.username
		AND phpbb_privmsgs_to.user_id = 9003
		ORDER BY Mtime DESC';	
	...
	while ($row = $db->sql_fetchrow($result)) {	
		echo ('<tr>');
			...
			echo ('<td style="word-wrap:break-word ">'.$row['message_text'].'</td>');
